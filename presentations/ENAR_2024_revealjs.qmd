...
![Link to slides: <https://lilykoff.github.io/ml_walking_fingerprint/>](figs/Fingerprinting.png){fig-align="left"}
...
![](figs/ClinicalTrial2.jpeg){.fragment width="400" height="420"}

![](figs/fall.png){.fragment width="400" height="420"}

![](figs/heart.jpg){.fragment width="400" height="420"}

![](figs/fign.jpg){.fragment width="400" height="420"}
:::
...
df_IU %>% filter(ID2 == 29 &
  ...
  geom_vline(aes(xintercept = 0), col = "black", linewidth = 1.1) + 
  geom_vline(aes(xintercept = 1), col = "black", linewidth = 1.1) + 
  geom_vline(aes(xintercept = 2), col = "black", linewidth = 1.1) + 
  geom_vline(aes(xintercept = 3), col = "black", linewidth = 1.1) + 
  geom_vline(aes(xintercept = 4), col = "black", linewidth = 1.1) + 
  geom_vline(aes(xintercept = 5), col = "black", linewidth = 1.1) + 
  geom_vline(aes(xintercept = 6), col = "black", linewidth = 1.1) + 
  geom_vline(aes(xintercept = 7), col = "black", linewidth = 1.1) + 
  geom_vline(aes(xintercept = 8), col = "black", linewidth = 1.1) + 
  geom_vline(aes(xintercept = 9), col = "black", linewidth = 1.1) + 
  geom_vline(aes(xintercept = 10), col = "black", linewidth = 1.1) 
...
+ For logistic regression models, use correlation and multiplicity adjusted (CMA) confidence intervals^[Ciprian M. Crainiceanu, Jeff Goldsmith, Andrew Leroux, and Erjia Cui. *Functional Data
Analysis with R*. Springer New York, NY, USA, 2023] for coefficients to identify grid cells that are most predictive of identity 

---

## Functional regression {.smaller}

+ Instead of summarizing joint distribution, use functional regression of the form: 
...
+ Indiana Unversity (IU)^[Karas, M., Urbanek, J., Crainiceanu, C., Harezlak, J., & Fadel, W. (2021). Labeled raw accelerometry data captured during walking, stair climbing and driving (version 1.0.0). *PhysioNet*. https://doi.org/10.13026/51h0-a262.]: 32 subjects, 8 min walking per subject
+ Zhejiang University (ZJU)^[Yuting Zhang, Gang Pan, Kui Jia, Minlong Lu, Yueming Wang, and Zhaohui Wu.
Accelerometer-Based Gait Recognition by Sparse Representation of Signature Points With
Clusters. *IEEE Transactions on Cybernetics*, 45(9):1864–1875, September 2015]: 153 subjects, two trials at least one week and up to six months apart, 1 min walking per subject
...
<table>
    ...
    <tr>
        ...
        <td>Rank-5 Accuracy</td>
        <td>Rank-1 Correct</td>
        <td>Rank-5 Correct</td>
    </tr>
    <tr>
        <th>IU</th>
        <td><small>Image partitioning - logistic</small></td>
        <td><FONT COLOR="#FFFF99">1.00</FONT></td>
        <td><FONT COLOR="#FFFF99">1.00</FONT></td>
        <td>32</td>
        <td>32</td>
    </tr>
    <tr>
        <th>IU</th>
        <td><small>Image partitioning - ML</small></td>
        <td>0.97</td>
        <td><FONT COLOR="#FFFF99">1.00</FONT></td>
        <td>31</td>
        <td>32</td>
    ...
    <tr>
        <th>ZJU S1</th>
        <td><small>Functional</small></td>
        <td><FONT COLOR="#FFFF99">0.98</FONT></td>
        <td><FONT COLOR="#FFFF99">1.00</FONT></td>
        <td>150</td>
        <td>153</td>
    </tr>
    ...
    <tr>
        <th>ZJU S1S2</th>
        <td><small>Image partitioning - ML</small></td>
        <td><FONT COLOR="#FFFF99">0.54</FONT></td>
        <td><FONT COLOR="#FFFF99">0.76</FONT></td>
        <td>82</td>
        <td>117</td>
    </tr>
    <tr>
...
### Results
# **Accuracies over Varying Amount of Testing Data**
## accuracies figure 
