...
read_text <- function(x){
  read.table(x, sep = ",") %>% t() %>% as.data.frame() %>%
    mutate(ID = sub("/rec.*", "", sub(".*subj_", "", x)), 
           session=sub("/subj.*", "", sub(".*gaitacc/", "", x)),
           record = sub(".txt.*", "", sub(".*rec_", "", x)))
}

# change your WD as necessary to get file list 
files <- c(list.files("/Users/lilykoffman/Documents/zju-gaitacc/session_0", recursive=T,
                      full.names=T),
           list.files("/Users/lilykoffman/Documents/zju-gaitacc/session_1", recursive=T,full.names=T),
           list.files("/Users/lilykoffman/Documents/zju-gaitacc/session_2", recursive=T,full.names = T))
# remove names we don't need 
vector <- c(grep("useful", files), grep("total", files), grep("cycles", files))
files <- files[-vector]
...
read_useful <- function(x){
  read.table(x, sep = ",") %>% rename("start" = V1, "end" = V2) %>% 
    mutate(session=sub("/subj.*", "", sub(".*gaitacc/", "", x)),
...
joined <- 
  all_data_zju_acc %>% 
  left_join(., useful_key, by = c("session"="session", "ID" = "ID", "rec" = "record" ))

# filter to just useful segment
filtered <- joined %>% filter(t >= start & t <= end) %>% dplyr::select(-c(start, end))
