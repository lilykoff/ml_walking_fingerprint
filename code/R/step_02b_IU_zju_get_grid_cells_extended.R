...
grid_data_lw_IU <- furrr::future_map_dfr(.x = subs_IU,
                           .f = get_grid_data,
                           time_lags = c(15, 30, 45, 60, 75, 90),
                           gcell_size = 0.25,
                           location = "lwrist",
                           data = df_all_IU,
                           .progress = T)
saveRDS(grid_data_lw_IU, file = here::here("data/grid_data_lw_IU_extended.rds"))

subs_zju <- unique(df_all_zju$ID[df_all_zju$session!="session_0"]) 

grid_data_rw_zju_s1 <- furrr::future_map_dfr(.x = subs_zju,
                               .f = get_grid_data,
                               time_lags = c(15, 30, 45, 60, 75, 90),
                               gcell_size = 0.25,
                               location = "rwrist",
...
grid_data_rw_zju_s2 <- furrr::future_map_dfr(.x = subs_zju,
                               .f = get_grid_data,
                               time_lags = c(15, 30, 45, 60, 75, 90),
                               gcell_size = 0.25,
                               location = "rwrist",
...
saveRDS(grid_data_rw_zju_s1, file = here::here("data/grid_data_rw_zju_s1_extended.rds"))
saveRDS(grid_data_rw_zju_s2, file = here::here("data/grid_data_rw_zju_s2_extended.rds"))
