...
onesig_lag <- function(data, id, s, lag){
  data %>% 
    filter(ID == id) %>%
    filter(second == s) %>%
    mutate(lagsig = dplyr::lag(signal_lwrist, lag),
           lag = lag)
}

lags <- rep(c(1, 15, 30, 45), 2)
seconds <- c(rep(1, 4), rep(2, 4))
...
df <-
  map2_dfr(data = df_all_IU, id = 19, 
           ...
           .x = seconds,
           .y = lags)
x.labs <- c("u = 1", "u = 15", "u = 30", "u = 45")
names(x.labs) <- c(1, 15, 30, 45)
...
df %>%
  ...
  scale_x_continuous(limits=c(0.75, 1.3), breaks = seq(0.75, 1.25, .25))+
  scale_y_continuous(limits=c(0.75, 1.3), breaks = seq(0.75, 1.25, .25))+
  geom_point()+
...
p1 <- 
  ggplot(data = gc_dat, aes(xmin  = lagsig, xmax = max_x,
                            ...
                            fill = n))+
  scale_fill_viridis(name = latex2exp::TeX(r'($X_{ijc}$)'))+
  geom_rect(alpha = 0.9)+
  facet_grid(second ~ lag, 
             labeller = labeller(second = y.labs, lag = x.labs))
...
p2 + 
  ...
  theme(axis.text.x = element_text(angle =30, vjust = .9))
...
secs <- c(1,2,5,10,15,20,25,30,35,50,60)
stats_ml <- 
  map(.x = secs,
      .f = get_accuracies,
      predictions = readRDS(here::here("predictions/IU_ml_predictions_revised.rds"))) %>%
  list_rbind() %>%
  mutate(
    model = "ml",
    data = "IU"
...
stats_logistic <- 
  map(.x = secs,
      .f = get_accuracies,
      predictions = readRDS(here::here("predictions/IU_logistic_predictions_revised.rds"))) %>%
  list_rbind() %>%
  mutate(
    model = "logistic",
    data = "IU"
...
secs <- c(1,2,5,10,15,20)
stats_ml <- 
  map(.x = secs,
      .f = get_accuracies,
      predictions = readRDS(here::here("predictions/zjus1_ml_predictions_revised.rds"))) %>%
  list_rbind() %>%
  mutate(
    model = "ml",
    data = "zjus1"
...
stats_logistic <- 
  map(.x = secs,
      .f = get_accuracies,
      predictions = readRDS(here::here("predictions/zjus1_logistic_predictions_revised.rds"))) %>%
  list_rbind() %>%
  mutate(
    model = "logistic",
    data = "zjus1"
...
stats_ml <- 
  map(.x = secs,
      .f = get_accuracies,
      predictions = readRDS(here::here("predictions/zjus1s2_ml_predictions_revised.rds"))) %>%
  list_rbind() %>%
  mutate(
    model = "ml",
    data = "zjus1s2"
...
all_accs %>%
  ...
  theme(strip.text = element_text(size = 12),
        ...
        aspect.ratio = 1)+
  facet_grid(metric~data, scales = "free_x")+
