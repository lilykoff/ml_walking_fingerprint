...
df_train <- get_functional_data(df_all_zju %>% filter(session=="session_1"), tlen = 1)
df_test <- get_functional_data(df_all_zju %>% filter(session=="session_2"), tlen = 1)
...
saveRDS(preds, "preds_functional_reg_zju_s1s2_new.rds")