...
get_grid_data  <- function(subject, time_lags, gcell_size, location, data){
  ...
  n_seconds <- max(df$second) # number of total seconds for the subject 
  ...
  get_grid_data_lagsec <- function(s, lag){
    ...
    df %>% filter(second==s) %>% 
      ...
      mutate(lag_hz = lag, ID = subject, second = s, cell = paste(cut_sig, cut_lagsig, lag_hz)) %>%
      dplyr::select(n, ID, second, cell) 
  }
  # apply above function over all seconds and lags, return df
  map2_dfr(.x = seconds, .y = lags,
           .f = get_grid_data_lagsec) %>% pivot_wider(id_cols = c(ID, second), names_from = cell, values_from = n) 
...
grid_data_lw_IU <- map_dfr(.x = subs_IU,
                        ...
                        data = df_all_IU,
                        .progress = T)
saveRDS(grid_data_lw_IU, file = "grid_data_lw_IU.rds")
...
saveRDS(grid_data_rw_zju_s1, file = "grid_data_rw_zju_s1.rds")
saveRDS(grid_data_rw_zju_s2, file = "grid_data_rw_zju_s2.rds")
